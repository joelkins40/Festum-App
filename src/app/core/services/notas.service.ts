...
export class NotasService {
	...
	private mockEventos: Evento[] = [
		...
		{
			id: 2,
			nombre: 'XV Años Ana María',
			fecha: new Date('2025-11-20'),
			lugar: 'Jardín Las Rosas',
		},
		{
			id: 3,
			nombre: 'Bautizo Sofía',
			fecha: new Date('2025-11-10'),
			lugar: 'Salón San Miguel',
		},
		{
			id: 4,
			nombre: 'Graduación Empresarial',
	...
	crearNota(
		dto: CrearNotaDto,
	): Observable<{ success: boolean; message: string; data?: Nota }> {
		this.loadingSubject.next(true);
	...
	getEstadisticas(): Observable<EstadisticasNotas> {
		...
		const promedioVenta =
			totalVentasMes > 0 ? totalIngresos / totalVentasMes : 0;
				...
				try {
					...
					dataLines.forEach((line: string) => {
						...
						if (values.length >= 8) {
							...
							const cliente =
								this.mockClientes.find((c) => c.id === clienteId) ||
								this.mockClientes[0];
							const evento =
								this.mockEventos.find((e) => e.id === eventoId) ||
								this.mockEventos[0];
