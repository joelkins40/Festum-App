...
} from '../../../core/models/productos-servicios.model';
...
export class ProductosServiciosComponent implements OnInit {
		...
		setTimeout(() => {
			...
			this.dataSource.filterPredicate = (
				...
				if (filterObj.texto) {
					...
					matches =
						...
						(data.nombre.toLowerCase().includes(searchText) ||
							...
							data.categoria?.descripcion?.toLowerCase().includes(searchText) ||
							false);
				}
				...
				if (filterObj.categoria) {
					matches =
						matches && data.categoriaId === parseInt(filterObj.categoria, 10);
				}
		...
		this.productosServiciosService
			.getProductosServicios({
				...
				categoriaId: this.filtroCategoria
					? parseInt(this.filtroCategoria, 10)
					: undefined,
			})
			.subscribe({
				next: (response) => {
					if (response.success) {
						this.mostrarMensaje(
							'Productos y servicios cargados exitosamente',
							'success',
						);
					}
				},
	...
	private crearProductoServicio(itemData: any): void {
		...
		this.productosServiciosService.crearProductoServicio(nuevoItem).subscribe({
			next: (response) => {
				if (response.success) {
					this.mostrarMensaje(
						`${nuevoItem.tipo} creado exitosamente`,
						'success',
					);
				} else {
	...
	private actualizarProductoServicio(itemData: any): void {
		...
		this.productosServiciosService
			.actualizarProductoServicio(itemActualizado)
			.subscribe({
				next: (response) => {
					if (response.success) {
						this.mostrarMensaje(
							`${itemActualizado.tipo} actualizado exitosamente`,
							'success',
						);
					} else {
						this.mostrarMensaje(response.message, 'error');
