...
export class NotasComponent implements OnInit {
  ...
  columnasDisplayed: string[] = ['tipo', 'folio', 'cliente', 'evento', 'fecha', 'total', 'estado', 'acciones'];
  filtroTexto = '';
  ...
  private configurarTabla(): void {
    setTimeout(() => {
      ...
      this.dataSource.filterPredicate = (data: Nota, filter: string) => {
        ...
        return data.folio.toLowerCase().includes(searchStr) ||
  ...
  abrirModalVer(nota: Nota): void {
    ...
    this.mostrarMensaje(`Folio: ${nota.folio} - Total: $${nota.total.toLocaleString('es-MX')}`, 'info');
  }
